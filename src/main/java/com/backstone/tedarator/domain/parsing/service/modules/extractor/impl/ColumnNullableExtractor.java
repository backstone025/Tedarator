...
public class ColumnNullableExtractor implements Extractor {
    @Override
    public boolean supports(AnnotationExpr annotation, boolean isPrimary, Type fieldType) {
        // 상황 1. 해당 필드가 PK일 경우 무조건 true 반환
        if (isPrimary) {
            return true;
        }

        // 상황 3. @Column 지정된 필드일 경우 true 반환
        return "Column".equals(annotation.getNameAsString());
    }

    @Override
    public AnnotationSnapshot<?> extract(AnnotationExpr annotation, boolean isPrimary, Type fieldType) {
        // JPA에서 기본적으로 true로 설정하기 때문이다.
        // primitive 타입의 경우, null 지정할 수 없기에 초기값을 지정해야 한다.
        boolean nullable = true;

        // 상황 1. isPrimary = true || isPrimitive = true
        // @Id로 지정되었으나, @Column 지정되어있지 않을 수 있기에 바로 반환하도록 구현했다.
        // 우선적으로 PK인지 판단하고, 그 다음 원시 타입인지 판별한다.
        if (isPrimary) {
            ...
            return new AnnotationSnapshot<>(
                    annotation != null ? annotation.getNameAsString() : "PrimitiveType",
                    ConstraintType.NULLABLE,
                    false
            );
        }
